...
class Trainer():
    def __init__(self, model, cfg):
        ...
        if cfg.verbose:
            ...
            #model = model(scale=cfg.scale, upscale=cfg.upscale, group=cfg.group)
            #flops, params = get_model_complexity_info(model, (3, 320, 180), as_strings=True, print_per_layer_stat=True)
            #print('Flops:  ' + flops)
            #print('Params: ' + params)
            ...
            for inputs in self.train_loader:
                ...
                if cfg.verbose and self.step % cfg.print_interval == 0:
                    with open('logs/{}/'.format(self.folder_name) + 'logs.txt', 'a') as f:
                        ...
                        f.write('Step: {}'
                                             '--> PSNR_x{}:{:.5f} -->{:.3f}m, Total_Loss: {:.5f}\n'
                                             .format(self.step, scale, mean_psnr, ((t2 - t1)/60), (self.mean_l1) / cfg.print_interval))
                       # f.write('Step: {}'
                       #                      '--> PSNR_x{}:{:.5f} -->{:.3f}m, Total_Loss: {:.5f}, a1: {:.5f}, a2: {:.5f}, b1: {:.5f}, b2: {:.5f}, b3:                    #          {:.5f}\n'
                       #                         .format(self.step, scale, mean_psnr, ((t2 - t1)/60), (self.mean_l1) / cfg.print_interval,
                       #                                 self.Network.x_scale.item(), self.Network.res_scale.item(), self.Network.x1.item(),                                #   self.Network.x2.item(), self.Network.x3.item()))
